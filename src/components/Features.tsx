...
export function Features() {
    return (
        <section data-theme="light" className="relative z-10 py-28 md:py-36 bg-[#FAFAF6]">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {features.map((f, i) => (
                        <motion.div
                            ...
                            className="bg-[#F0EDE4] p-8 md:p-10 flex flex-col gap-5 group border-t-[3px] border-[#C9A84C] rounded-b-2xl hover:shadow-[0_0_30px_rgba(201,168,76,0.2)] transition-all"
                        >
                            <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center text-[#C9A84C] shadow-sm group-hover:scale-110 transition-transform border border-black/5">
                                <f.icon />
                            </div>
                            <h3 className="text-xl font-black tracking-tight text-black">{f.title}</h3>
                            <span className="font-telugu text-xs text-[#C9A84C] italic -mt-3">{f.telugu}</span>
                            <p className="text-sm text-black/55 leading-relaxed font-medium">{f.desc}</p>
                        </motion.div>
                    ))}
                </div>
            </div>
